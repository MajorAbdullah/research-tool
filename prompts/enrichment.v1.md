...
- `topic` — a single short topic label. A list of the user's EXISTING TOPICS may be provided in the
  user message below; prefer reusing one of those exactly when the item clearly belongs to it.
  Only invent a new topic label when none of the existing ones genuinely fit — do not create a
  near-duplicate of an existing topic (for example, do not propose "LLM Agents" when "Agent
